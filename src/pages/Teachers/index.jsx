...
export function Teachers() {
  ...
  function handleNavigate() {
    navigate("/teacherscreate")
  }

  function handleNavigateUpdate(id) {
    navigate(`/teachersupdate/${id}`)
  }
  ...
  useEffect(() => {
    async function fetchTeachers() {
      const response = await api.get(`/teachers/?name=${search}`)
    ...
    <ContainerOne>
      <ContainerTwo className={statePage ? "containerLight" : "containerDark"}>
        ...
        <div style={{ 
          ...
          <h1 style={{ color: statePage ? "#00111A" : "#FFFAF1" }}>Docente</h1>
            ...
            <tbody className={statePage ? "tableLight" : "tableDark"}>
              {
                teachers.map((teacher) => (
                  <tr class="rows">
                    ...
                    <td class="cpf-row">{teacher.cpf}</td>
                    <td class="date-row">{formatDate(teacher.birth)}</td>
                    <td class="options-row">
                      <a class="change" onClick={() => handleNavigateUpdate(teacher.id)}>Alterar</a>
                      <a class="remove" onClick={() => deleteTeacher(teacher.id)}>Excluir</a>
                    </td>
                  </tr>
                ))
              }
