...
export function TeachersUpdate() {
  ...
  useEffect(() => {
    async function fetchTeacher() {
      ...
      setName(response.data.teacher[0].name)
      setEmail(response.data.teacher[0].email)
      setCpf(response.data.teacher[0].cpf)
      setDate(response.data.teacher[0].birth)
      setPassword(response.data.teacher[0].password)
    }
    ...
    <ContainerOne>
      <ContainerTwo className={statePage ? "containerLight" : "containerDark"}>
        ...
        <Form className={statePage ? "light" : "dark"}>
          <h3 className={statePage ? "light" : "dark"} style={{ color: statePage ? "#00111A" : "#FFFAF1" }}>Alterar Docente</h3>
          ...
          <div style={{display: "flex", width: "100%", justifyContent: "center"}}>
            <Button type="submit" onClick={(e) => updateTeacher(e)}>Alterar Usuário</Button>
          </div>
        </Form>
      </ContainerTwo>
    </ContainerOne >
