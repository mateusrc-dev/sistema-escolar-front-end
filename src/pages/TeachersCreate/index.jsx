...
export function TeachersCreate() {
  ...
  function createNewTeacher(e) {
    ...
    api.post("/teachers", { 
      ...
      alert("Professor cadastrado com sucesso!"); navigate("/teachers") 
    }).catch(error => { 
      if (error.response) { 
    ...
    <ContainerOne>
      <ContainerTwo className={statePage ? "containerLight" : "containerDark"}>
        ...
        <Form className={statePage ? "light" : "dark"}>
          <h3 style={{ color: statePage ? "#00111A" : "#FFFAF1" }} className={statePage ? "light" : "dark"}>Cadastro Docente</h3>
          ...
          <div style={{display: "flex", width: "100%", justifyContent: "center"}}>
            <Button type="submit" onClick={(e) => createNewTeacher(e)}>Criar Usuário</Button>
          </div>
        </Form>
      </ContainerTwo>
    </ContainerOne >
