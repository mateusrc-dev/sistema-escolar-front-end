...
export function StudentsUpdate() {
  ...
  useEffect(() => {
    async function fetchStudent() {
      ...
      setEmail(response.data.student[0].email)
      setCpf(response.data.student[0].cpf)
      setDate(response.data.student[0].birth)
      setPassword(response.data.student[0].password)
    }
    ...
    <ContainerOne>
      <ContainerTwo className={statePage ? "containerLight" : "containerDark"}>
        ...
        <Form className={statePage ? "light" : "dark"}>
          <h3 style={{ color: statePage ? "#00111A" : "#FFFAF1" }} className={statePage ? "light" : "dark"}>Alterar Aluno (a)</h3>
          ...
          <div style={{display: "flex", width: "100%", justifyContent: "center"}}>
            <Button type="submit" onClick={(e) => updateStudents(e)}>Alterar Usuário</Button>
          </div>
        </Form>
      </ContainerTwo>
    </ContainerOne >
